...
public class RemoveStudentPane {
	private GridPane gridPane;
	private BorderPane studentPane;

	public RemoveStudentPane(College college) {
		...
		Text text1 = new Text();
		text1.setText("Remove a Student");
		...
		okBtn.setOnAction(event2 -> {
			String studentID = removeIDField.getText();
			Person student1 = college.getPersonBag().findById(studentID);
			int studentToRemove = college.getPersonBag().removeById(studentID);
			if (studentToRemove != -1 && student1 instanceof Student) {
				Alert confirmation = new Alert(AlertType.CONFIRMATION, "Student "+ student1.getName() + " Removed!", ButtonType.OK);
